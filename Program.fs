...
let main argv =
    ...
    | Optional "d" (dir, Optional "de" (de, Optional "df" (df, Optional "f" (fileFilter, Optional "without"(without, Replace(replacements, (_::_ as patterns))))))) ->
        let rootDir =
            match dir with
        ...
        let accept =
            match without with
            | None -> fun line -> true // always accept if there's no without clause
            | Some without -> isMatch without >> not
        let rec recur dir =
            for file in Directory.EnumerateFiles dir |> Seq.filter pathFilter do
                ...
                if File.ReadLines file |> Seq.filter (fun ln -> accept ln && patterns |> Seq.every(flip isMatch ln)) |> Seq.isEmpty |> not then
                    ...
                    try
                        if replacements |> Seq.isEmpty then
                            for line in File.ReadLines file |> Seq.filter (fun ln -> accept ln && patterns |> Seq.every(flip isMatch ln)) do
                                printfn "%s" line
                        else
                        ...
                        printfn ""
                    with err ->
                        printfn $"Skipping due to error: '{err}'"
            for d in Directory.EnumerateDirectories dir |> Seq.filter directoryFilter do
                recur d
        recur rootDir
    | _ -> printfn "Usage: matchLines [-d <directory>] [-df <directoryFilter>] [-de <directories to exclude>] [-f <pathFilter>] [-replace [<pattern>] <value>] <patterns...> [without <pattern to exclude>]"
